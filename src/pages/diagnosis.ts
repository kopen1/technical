...
.vref{border:1px solid #eaecf0;border-radius:14px;padding:14px;margin:14px 0}.vref .imgbox{position:relative;display:inline-block;max-width:100%;line-height:0}.vref img{max-width:100%;height:auto;border-radius:10px}
...
let session=null,step=null,total=0;
...
function renderVisuals(visuals){
 ...
 return \`<h2>Visual Reference</h2>\`+visuals.map(v=>{
  ...
  return \`<div class="vref"><div class="imgbox"><img src="\${esc(v.url||"")}" alt="\${esc(v.caption||"")}">\${markers}</div><p><b>\${esc(v.caption||"")}</b> \${srcBadge(v.verificationStatus)} <span class="badge b-community">\${esc(VT_LABEL[v.imageType]||v.imageType)}</span></p>\${v.source?\`<p class="muted">Sumber: \${esc(v.source)}</p>\`:""}</div>\`;
 }).join("");
}
(async()=>{
...
async function begin(){
 ...
 const d=await r.json();session=d.session;step=d.step;total=d.total||0;update();render();
}
function render(){
 ...
 else{run.innerHTML=\`<h3>\${esc(step.title)}</h3><p>\${esc(step.instruction)}</p><p class="muted"><b>Metode:</b> \${esc(step.method)}\${step.testPoint?\` · <b>Test Point:</b> \${esc(step.testPoint)}\`:""}</p><input id="measurement" placeholder="Masukkan hasil pemeriksaan"><button class="btn" onclick="nextStep()">Lanjut</button>\`}
 evlog.innerHTML=session.evidence.length?\`<h3>Evidence log</h3>\`+session.evidence.map(e=>\`<div class="log"><b>\${esc(e.stepTitle)}</b><br>\${esc(e.value)}</div>\`).join(""):"";
}
function update(){}
